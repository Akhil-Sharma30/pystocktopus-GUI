...
def main():
    ...
    def show_welcome_screen():
        ...
        button1 = tk.Button(
            root,
            text="Generate a Stock Closing Price CSV",
        ...
        button2 = tk.Button(
            root,
            text="Prediction of Next Day Closing Price",
        ...
        button3 = tk.Button(
            root,
            text="News Analysis for a Stock",
        ...
        def generate_csv():
            ...
            try:
                ...
                data = StockExtractor.ticker_data_collection(
                    ticker_values=[names[0]],
                    timespan=names[1],
                    multiplier=int(names[2]),
                    user_date=start_date.strip(),
                    days=int(names[3]),
                )
                print(data)
                CSVDataHandler.close_list_csv(data, csv_file_name=filename)
                result_label.config(
                    text=f"Success: CSV file '{filename}' has been generated successfully.",
                    fg="green",
                )
            except Exception as e:
                messagebox.showerror(
                    "Error", f"Failed to generate CSV file. Error: {e}"
                )
    ...
    def show_button_screen_PRED():
        ...
        name_labels = [
            ...
            "Stock Closing Price Column Name in CSV:",
        ]
        name_entries = []
        ...
        def pred_value():
            names = [
                entry.get() for entry in name_entries
            ]  # Get all user inputs from the entry fields
            ...
            if not names[0] or not file:
                messagebox.showerror(
                    "Error", "Please enter all fields and upload a file."
                )
                return
            ...
            try:
                ...
                messagebox.showerror(
                    "Error", "Please enter valid numbers for Epochs and Learning Rate."
                )
                return
            ...
            def run_training():
                try:
                    ...
                    result_label.config(
                        text=f"News analysis completed: {prediction}", fg="green"
                    )
    ...
    def show_button_screen_NEWS():
        ...
        start_date_entry = DateEntry(
            input_frame, date_pattern="yyyy-mm-dd"
        ...
        def analyze_news():
            ...
            try:
                ...
                messagebox.showerror(
                    "Error", "Please enter a valid number for Time stamp."
                )
                return

            # Now call the News extraction function with the user inputs
            try:
                result = News.new_data_extract(
                    ticker_values=[names[0]], predict_date=start_date, days=days
                )
                print(result)
